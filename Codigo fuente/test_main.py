...
class TestMainFunctions(unittest.TestCase):
    ...
    def test_validacion_fragmentos_insuficientes(self):
        """ Verificar que falla con fragmentos insuficientes """
        n, t = 5, 3
        contraseña = "contraseña_prueba"
        _, fragmentos = generar_fragmentos(contraseña, n, t)

        fragmentos_insuficientes = fragmentos[:2]
        with self.assertRaises(ValueError):
            reconstruir_secreto(fragmentos_insuficientes)

    def test_encriptar_sin_archivo(self):
        """ Verificar que no encripta si el archivo no existe """
        clave = "clave_inexistente"
        archivo_inexistente = "archivo_inexistente.txt"

        with self.assertRaises(FileNotFoundError):
            encriptar_archivo(archivo_inexistente, clave)

    def test_descifrar_sin_archivo(self):
        """ Verificar que no descifra si el archivo cifrado no existe """
        clave = "clave_inexistente"
        archivo_inexistente = "archivo_inexistente_encriptado.txt"

        with self.assertRaises(FileNotFoundError):
            desencriptar_archivo(archivo_inexistente, clave)
